...
class LandmarkCounterHeap {
    companion object {
        ...
        fun insert(landmark: Landmark) {
            ...
            while (DistanceHelper.getDistace(heap[current].getLatLong(), userLoc) < DistanceHelper.getDistace(heap[parent(current)].getLatLong(), userLoc)) {
        ...
        fun landmarkCanBeVisited() : Boolean = (DistanceHelper.getDistace(peekTop().getLatLong(), userLoc) <= distanceMax)
    }
}