...
class StepsDataInstrumentedTest {
    ...
    fun testValidLocationValue()
    {
        ...
        StepData.locationsVisited = locVal
        Assert.assertEquals(locVal, StepData.locationsVisited)
    }

    @Test(expected = IllegalArgumentException::class)
    fun testInvalidLocationValueTooSmall()
    {
        StepData.locationsVisited = -1
    }

    @Test(expected = IllegalArgumentException::class)
    fun testInvalidLocationValueTooLarge()
    {
        StepData.locationsVisited = Landmarks.landmarks.size + 1
    }
}